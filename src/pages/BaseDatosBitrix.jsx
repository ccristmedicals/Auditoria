...
const ClassificationBadge = ({ value }) => {
    ...
    const colorClass = colorMap[letter] || "bg-gray-100 text-gray-400 border-gray-200 dark:bg-gray-800";

    return (
        <div className="flex justify-center">
            <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold border ${colorClass} transition-colors`}>
                {letter || "-"}
            </span>
        </div>
    );
...
const BaseDatosBitrix = () => {
    ...
    const [selectedIds, setSelectedIds] = useState([]); // IDs de filas marcadas
    const [showOnlySelected, setShowOnlySelected] = useState(false); // Switch de filtro
    ...
    // Filtrar data para el renderizado
    const displayedCompanies = showOnlySelected
        ? companies.filter(c => selectedIds.includes(c.id_interno))
        : companies;
    ...
    const handleSave = async (companyData) => {
        const payload = {
            ...
            gestion: {
                ...
                semana: {
                    lunes: { accion: companyData.lunes_accion, ejecucion: companyData.lunes_ejecucion },
                    martes: { accion: companyData.martes_accion, ejecucion: companyData.martes_ejecucion },
                    miercoles: { accion: companyData.miercoles_accion, ejecucion: companyData.miercoles_ejecucion },
                    jueves: { accion: companyData.jueves_accion, ejecucion: companyData.jueves_ejecucion },
                    viernes: { accion: companyData.viernes_accion, ejecucion: companyData.viernes_ejecucion },
                }
            },
        ...
        <div className="p-4 sm:p-6 lg:p-8 min-h-screen bg-white dark:bg-[#191919]">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-[#191919] dark:text-white">Auditoría de Clientes</h2>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                            Total: {totalRecords} | Seleccionados: {selectedIds.length}
                        </p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-medium border transition-all flex items-center gap-2 shadow-sm ${showOnlySelected
                    ...
                    >
                        <Filter size={16} />
                        {showOnlySelected ? "Mostrar Todos" : `Ver Seleccionados (${selectedIds.length})`}
                    </button>

                    <div className="px-3 py-1 bg-red-50 text-red-600 rounded-lg border border-red-100 dark:bg-red-900/20 dark:border-red-800 flex items-center gap-2 text-xs">
                        <AlertTriangle size={14} />
                        <span>Errores: <strong>{companies.filter(c => !c.codigo_profit || c.codigo_profit === "0").length}</strong></span>
                    </div>
                </div>
            </div>
                    ...
                    <Table>
                        <Thead>
                            <Tr className="bg-gray-50 dark:bg-[#1e1e1e] border-b-0">
                                {/* Checkbox Global */}
                                <Th stickyLeft className="bg-white dark:bg-[#191919] w-12 text-center border-r dark:border-[#333]">
                                    <input
                                        type="checkbox"
                            ...
                            <Tr>
                                <Th stickyLeft className="bg-white dark:bg-[#191919] border-r dark:border-[#333]"></Th>
                                <Th stickyLeft className="min-w-[60px] bg-white dark:bg-[#191919]">ID</Th>
                                <Th stickyLeft className="min-w-[200px] text-left bg-white dark:bg-[#191919] shadow-md">Nombre</Th>
                                {/* ... Resto de cabeceras igual que en tu código original ... */}
                        ...
                        <Tbody>
                            {displayedCompanies.map((row) => (
                                <Tr key={row.id_interno} className={selectedIds.includes(row.id_interno) ? "bg-teal-50/30 dark:bg-teal-900/10" : ""}>
                                    {/* Celda de Checkbox */}
                                    <Td stickyLeft className="bg-white dark:bg-[#191919] border-r dark:border-[#333] text-center">
                                        <input
                                            type="checkbox"
                                            className="w-4 h-4 accent-[#1a9888] cursor-pointer"
                                    ...
                                    <Td stickyLeft className="text-left font-semibold bg-white dark:bg-[#191919] border-r shadow-md">
                                        <div className="flex items-center gap-2 truncate max-w-[280px]" title={row.nombre}>
                                            {row.nombre}
                                        </div>
                                    </Td>
                                    ...
                                    <Td>{row.sku_mes}</Td>
                                    <Td><ClassificationBadge value={row.clasificacion} /></Td>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between border-t border-gray-200 dark:border-gray-700 pt-4 mt-2 gap-4">
                <div className="flex items-center gap-2">
                    <button onClick={() => goToPage(1)} disabled={page === 1} className="p-2 rounded-lg border dark:border-gray-600 disabled:opacity-50"><ChevronsLeft size={16} /></button>
                    <button onClick={() => goToPage(page - 1)} disabled={page === 1} className="flex items-center gap-1 px-3 py-2 text-sm border rounded-lg disabled:opacity-50 dark:bg-[#333] dark:text-gray-200"><ChevronLeft size={16} /> Anterior</button>
                    <span className="px-4 py-2 text-sm font-semibold text-[#1a9888] bg-teal-50 dark:bg-teal-900/20 rounded-lg">Página {page} de {totalPages}</span>
                    <button onClick={() => goToPage(page + 1)} disabled={page === totalPages} className="flex items-center gap-1 px-3 py-2 text-sm border rounded-lg disabled:opacity-50 dark:bg-[#333] dark:text-gray-200">Siguiente <ChevronRight size={16} /></button>
                    <button onClick={() => goToPage(totalPages)} disabled={page === totalPages} className="p-2 rounded-lg border dark:border-gray-600 disabled:opacity-50"><ChevronsRight size={16} /></button>
                </div>
                ...
                <form onSubmit={handleJumpSubmit} className="flex items-center gap-2">
                    ...
                    />
                    <button type="submit" className="px-3 py-1 text-xs font-bold text-white bg-gray-600 rounded-md">IR</button>
                </form>
            </div>
        </div>
    );
