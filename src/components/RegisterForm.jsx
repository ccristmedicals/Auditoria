      ...
      <div className="mb-8" ref={segmentRef}>
        ...
        <div className="relative">
          <div
            ...
            className={`w-full min-h-[42px] pl-10 pr-10 py-2 border-2 rounded-lg cursor-pointer bg-white dark:bg-[#2b2b2b] border-gray-300 dark:border-gray-600 flex flex-wrap gap-2 items-center transition-all duration-200 ${isSegmentMenuOpen
          ...
          >
            ...
            {formData.segmentos.length === 0 ? (
              ...
              formData.segmentos.map((segCode, idx) => {
                ...
                return (
                  <span
                    key={idx}
                    className="bg-teal-100 text-teal-800 text-xs font-semibold px-2 py-1 rounded-full flex items-center gap-1 dark:bg-teal-900 dark:text-teal-100"
                  >
                    {displayName}
                    <button
                      type="button"
            ...
            <span className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-gray-500 h-full">
              <ChevronDown
                size={20}
                className={`transition-transform duration-200 ${isSegmentMenuOpen ? "rotate-180" : ""
                  }`}
              />
            </span>
          </div>
              ...
              {availableSegmentos.map((seg, idx) => {
                ...
                return (
                  ...
                  >
                    <span>{name}</span>
                    {isSelected && (
                      <Check size={16} className="text-[#1a9888]" />
                    )}
                  </div>
                );
              })}
